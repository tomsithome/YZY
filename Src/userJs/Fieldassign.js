layui.config({
    ...
    var mainList = function(options) {
        ...
        table.reload('mainList',
            {
                url: '/Field/LoadTable?tablename='+tablename+'',
                where: config,
                done: function(res, curr, count) {
                    //如果是异步请求数据方式，res即为你接口返回的信息。
                    //如果是直接赋值的方式，res即为：{data: [], count: 99} data为当前页数据、count为数据总长度
                    var url = "/Field/LoadForUser";
                    if (type.indexOf("Role") != -1) {
                        url = "/Field/LoadForRole";
                    }
                            ...
                            ,success: function(json) {
                                ...
                                for (var i = 0; i < res.data.length; i++) {
                                    for (var j = 0; j < roles.length; j++) {
                                        if (res.data[i].name != roles[j]) continue;
    ...
    table.on('checkbox(list)', function (obj) {
        ...
        var url = "/RelevanceManager/AssignField";
        if (!obj.checked) {
            url = "/RelevanceManager/UnAssignField";
        }
        $.post(url, { type: type, firstId: id, secIds: [obj.data.name], tablename: tablename}
                       , function (data) {

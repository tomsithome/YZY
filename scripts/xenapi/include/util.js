...
Util.get_include_uri = function() {
    return (typeof INCLUDE_URI !== "undefined") ? INCLUDE_URI : "/scripts/xenapi/include/";
}
...
Util.getEventPosition = function (e, obj, scale) {
    ...
    //console.log("evt.clientX.x:" + evt.clientX + ",e.y:" + evt.clientY);
    //console.log("document.scrollTop:" + $(document).scrollTop());
    evt = (evt.changedTouches ? evt.changedTouches[0] : evt.touches ? evt.touches[0] : evt);
    var $canvas = $("#noVNC_canvas");
    var $screen = $("#noVNC_screen");
    var element = $canvas[0];
    if (evt.pageX || evt.pageY) {
        docX = evt.pageX;
        docY = evt.pageY + $("#responsive").scrollTop() - $(document).scrollTop();
       
        if (element.msRequestFullscreen) {
            if ($canvas.css("position") !== "static") {
                var marginLeft = parseInt($(window).width() / 2 - $canvas.width() / 2);
                docX -= marginLeft;
                console.log(marginLeft);
            }
        }
        
    } else if (evt.clientX || evt.clientX) {
        docX = evt.clientX + document.body.scrollLeft +
            document.documentElement.scrollLeft;
        docY = evt.clientY + document.body.scrollTop +
            document.documentElement.scrollTop + $("#responsive").scrollTop() - $(document).scrollTop();
    }
    if (element.mozRequestFullScreen && $screen.data("isfull") == 1) {
        //alert($(window).width() + "," + $canvas.width());
        var scale = window.screen.height / $canvas.attr("height");
        var mozWidth = $canvas.attr("width") * scale;
        var marginLeft2 = parseInt($(window).width() / 2 - mozWidth / 2);
        docX -= marginLeft2;
    }

    //console.log($screen.data("isfull"));
    ...
    if ($canvas.height() !== obj.height) {
        scale = $canvas.height() / obj.height;
    }
    //console.log($canvas.height() + "," + obj.height);
    //console.log(scale);
    ...
    if (docY - y - pos.y > 0)
        y += docY - y - pos.y;
    return { 'x': parseInt(x / scale), 'y': parseInt(y / scale) };
};
